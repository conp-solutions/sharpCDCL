...
#include "minisat/core/VarFileParser.h"  // to parse the white file



using namespace Minisat;

extern StringOption  opt_projection;
...
int main(int argc, char** argv)
{
	...
	/** found projection file -> freeze variables! */
	if(  opt_projection != 0) {
	  // read in projection variables
	  //projectionVariables
	  VarFileParser parse ( (string)opt_projection );  // open file for parsing
	  std::vector<int> projectionVariables;
	  int max = parse.extract(projectionVariables);
	  max = max >= S.nVars() ? max : S.nVars();
	  char* isInProjection = (char*) malloc( sizeof(char) * max + 1 );
	  ...
	  for( int i = 0 ; i < projectionVariables.size(); ++i ) {
	    ...
	    S.setFrozen( projectionVariables [i], true );
	  }
 	    printf("|  Froze               %12d variables                                 |\n", projectionVariables.size() );
	}
